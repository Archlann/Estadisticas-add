...
agrupado = sr.groupby('item_name')['quantity'].sum().reset_index()

plt.bar(agrupado['item_name'], agrupado['quantity'])
...
sr['date'] = sr['date'].str.replace('-', '/')  # Cambia "07-03-2022" a "07/03/2022"
sr['date'] = pd.to_datetime(sr['date'], errors='coerce', dayfirst=True)

# 1. Ventas totales por tipo de ítem
ventas_por_tipo = sr.groupby('item_type')['transaction_amount'].sum().reset_index()

# Gráfico 1: Ventas totales por tipo de ítem
plt.figure(figsize=(8,5))
sns.barplot(data=ventas_por_tipo, x='item_type', y='transaction_amount', palette='viridis')
plt.title('Ganancias totales por tipo de item')
plt.ylabel('Ganancia total de transacciones')
